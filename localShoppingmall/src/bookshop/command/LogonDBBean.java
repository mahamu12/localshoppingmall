...
public class LogonDBBean {
	...
	public LogonDataBean getMember(String id) {
		...
		return member; //데이터 저장빈 객체 memeber 리턴
	}
	...
	public int updateMemeber (LogonDataBean member) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		int x = -1;
		...
		try {
			...
			if(rs.next()) {//해당 아이디가 있으면수행
				String dbpasswd = rs.getString("passwd");
				if(passwd == dbpasswd) {
					pstmt = conn.prepareStatement(
							"update memeber set name = ?,address = ? ,tel = ? where id =?");
